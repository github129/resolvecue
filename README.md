...
`discriminator` はエフェクト固有の識別子。矢印では方向略称 (`tr`/`r`/`br`/`b`/`bl`/`l`/`tl`/`t`/`c`)。
discriminator を持たないエフェクトでは省略され `cue_<effect>_<frame>` となる。

同じフレーム + 同じ方向の組み合わせを「同一エフェクト」とみなす。同じマーカーに違う方向の矢印を
複数本立てたい場合は別の方向プリセットで再実行すれば衝突しない。

ユーザーがクリップ名を変更しなければ、再実行時の上書き / スキップが正しく機能します。
